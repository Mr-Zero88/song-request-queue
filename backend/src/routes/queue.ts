...
import {
	Queue,
	GetQueueRequestParams,
	AddToQueueRequestParams,
	AddToQueueRequestBody,
	RemoveFromQueueRequestParams,
	RemoveFromQueueRequestBody,
} from "song-request-queue-common/types/queue";
...
interface AddToQueueRequest extends Request<
	AddToQueueRequestParams,
	Queue,
	AddToQueueRequestBody
> {}
...
export async function addToQueue(req: AddToQueueRequest) {
	let queue = await controllers
		.addToQueue(req.params.id, req.body?.link)
		.catch(handleError);
	if (queue instanceof Error) return req.next(queue);
	req.res.status(200).json(queue);
}
...
export async function removeFromQueue(req: RemoveFromQueueRequest) {
	let queue = await controllers
		.removeFromQueue(req.params.id, req.body?.link)
		.catch(handleError);
	if (queue instanceof Error) return req.next(queue);
	req.res.status(200).json(queue);
}

...
export async function getSession(req: Request, res: Response, next: NextFunction) {
    let session = await controllers.getSession({ id: req.cookies.sessionId }).catch(handleError);
    if(session instanceof Error) return next(session);
    res.status(200).json(session);
}

export async function createSession(req: Request, res: Response, next: NextFunction) {
    let session = await controllers.createSession(req.body).catch(handleError);
    if(session instanceof Error) return next(session);
